...
import { ArrowLeft, BookOpen, User, Calendar, CheckCircle, AlertCircle, Wrench } from 'lucide-react';
...
const BookDetails: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      {/* Navigation */}
      <Button variant="ghost" onClick={() => navigate('/livros')}>
        <ArrowLeft className="mr-2 h-4 w-4" />
        Voltar para Livros
      </Button>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="flex gap-3 pt-4 border-t border-border">
            {book.status === 'Disponível' && (
              <Button onClick={handleLoan} className="gap-2">
        ...
        <CardContent>
          {book.loanHistory.length > 0 ? (
            <div className="space-y-3">
              {book.loanHistory.map((entry, index) => (
                <div
                  key={index}
